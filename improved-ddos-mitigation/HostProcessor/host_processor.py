...
def save_block_history(host_config: HostDetails, blocked_ips: List[str], ip_type: str, ipset_id: str, ipset_name: str) -> None:
    ...
    for ip in blocked_ips:
        ...
        if not response.get('Item'):
            print(f"Put block ip to DDB table {WAF_BLOCK_IP_TABLE} data: {values}")
            table.put_item(
                Item=values
            )
            publish_to_sns(host_config, ip, ip_type)
    print("Added %s ip in Dynamo DB table %s", blocked_ips, WAF_BLOCK_IP_TABLE)
...
def update_waf_ipset(ipset_name: str, ipset_id: str, ips_to_be_blocked: Set[str], host_config: HostDetails, ip_type: str, retry: int = 3) -> Tuple[List[str], str]:
    ...
    print("ips_to_be_blocked 1 = ",ips_to_be_blocked)
    # ips_to_be_blocked.discard('-')
    # print("ips_to_be_blocked 2 = ",ips_to_be_blocked)
    if ip_type == "IPv4":
    ...
    print("Blocking: %s", str(ips_to_be_blocked_list))
    ...
    try:
        ...
        logger.error(e)
        if retry > 0:
            print("Retrying Function Update Waf IpSet: %d", retry)
    ...
    print("Added Offending Ip Addresses to WAF IPSets")
...
def process_row(column_info: List[str], row: Dict[str, Any], threshold: int) -> Tuple[Optional[Union[str, List[str]]], Optional[str]]:
    ...
    try:
        ...
        if int(row_dict["Frequency"]) > threshold:
            print("Row debug: %s", str(row_dict))
...
def process_host(header: str, host: str, duration: str, threshold: int, offending_ips: IpDetails) -> IpDetails:
    ...
    try:
        ...
        # print("TimeStream Query Success")
...
def publish_to_sns(host_config: HostDetails, ip: str, ip_version: str) -> None:
    ...
    print("Published to SNS: %s", str(response))
...
def get_config(host: str, distribution: str) -> Tuple[str, int]:
    ...
    print(f"Configurations details from Dynamo db: {response}")
...
def check_ip(ips: Set[str], ip_version: str) -> bool:
    ...
    if ips:
        print("Found %d new %s Addresses", len(ips), ip_version)
        return True
    else:
        print("No %s Addresses found", ip_version)
        return False
...
def lambda_handler(event: str, context) -> None:
    ...
    print(f"{host_config} from table {WAF_DDB_CONFIG_TABLE}")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=THREAD_COUNT) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_project):
            ...
            try:
                ...
                print("%s query status  %s", processed, data)

    print(f"offending_ips_info: {offending_ips_info}")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=THREAD_COUNT) as executor:
        ...
        for future in concurrent.futures.as_completed(future_to_project):
            ...
            try:
                ...
                print("%r status  %s", processed, data)
                # print("Updated IPSet %s with %d IP's", ip_details["ipset_name"], len(data))
                print('Blocked Ips: %s', str(data))
